...
impl BorrowableMedia {
    ...
    fn borrow(&mut self, quantity: Option<u32>) -> Result<(), String> {
        ...
        if self.copies_in_stock >= quantity {
            ...
            Err(format!(
                "Not enough copies available. Available: {}, Requested: {}",
                self.copies_in_stock, quantity
            ))
        }
    }

    fn return_item(&mut self, quantity: Option<u32>) -> Result<(), String> {
        let quantity = quantity.unwrap_or(1);
        if self.copies_borrowed >= quantity {
            ...
            Err(format!(
                "Cannot return more copies than borrowed. Borrowed: {}, Requested: {}",
                self.copies_borrowed, quantity
            ))
        }
    }
            ...
            "6" => {
                ...
                if let Ok(uuid) = Uuid::parse_str(&uuid) {
                    if let Some(item) = library.items.get_mut(&uuid) {
                        match item {
                            Item::Book(book) => {
                                if let Ok(_) = book.borrowable_media.borrow(None) {
                                    println!("Returned Book: {:#?}", book);
                                } else {
                                    println!("No copies returned.");
                                }
                            }
                            Item::AudioBook(audiobook) => {
                                match audiobook.borrowable_media.borrow(None) {
                                    Ok(()) => {
                                        println!("Returned Audiobook: {:#?}", audiobook);
                                    }
                                    Err(error_string) => {
                                        println!("{} - No copies returned.", error_string);
                                    }
                                }
                            }
            ...
            "7" => {
                ...
                if let Ok(uuid) = Uuid::parse_str(&uuid) {
                    if let Some(item) = library.items.get_mut(&uuid) {
                        match item {
                            Item::Book(book) => {
                                if let Ok(_) = book.borrowable_media.return_item(None) {
                                    println!("Returned Book: {:#?}", book);
                                } else {
                                    println!("No copies returned.");
                                }
                            ...
                            Item::AudioBook(audiobook) => {
                                match audiobook.borrowable_media.return_item(None) {
                                    Ok(()) => {
                                        println!("Returned Audiobook: {:#?}", audiobook);
                                    }
                                    Err(error_string) => {
